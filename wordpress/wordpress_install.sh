...
FIRST_IFACE=1 && CHECK_CURRENT_REPO=1 . <(curl -sL https://raw.githubusercontent.com/vivian-src/services-vm/master/wordpress/lib.sh)
unset FIRST_IFACE
...
if ! version 20.04 "$DISTRO" 20.04.6; then
    print_text_in_color "$IRed" "Ubuntu version $DISTRO must be between 20.04 - 20.04.6"
    exit
fi
